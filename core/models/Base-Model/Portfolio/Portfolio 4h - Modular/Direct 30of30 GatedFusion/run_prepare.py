...
from lib import news as N, market as M, features as F
...
def main():
    ...
    if os.path.exists(P.news_pickle):
        df_news = pd.read_pickle(P.news_pickle); print('Loaded cached news embeddings.')
        ...
        df_news.to_pickle(P.news_pickle); print('Saved news embeddings.')
    ...
    for sym in MC.symbols_usdt:
        ...
        if os.path.exists(fpath): df1m = pd.read_pickle(fpath)
        else:
        ...
        per_asset.append(F.add_targets_and_features(df3m, FC.horizon_steps, FC.seq_len, None, sym))
    merged = F.merge_assets(per_asset); merged = F.attach_news(merged, news_3m)
    tcols = F.make_time_cols(merged); merged = pd.concat([merged, tcols], axis=1).fillna(0)
    os.makedirs(os.path.dirname(P.merged_parquet), exist_ok=True)
    merged.to_parquet(P.merged_parquet, index=False); print('Saved merged 3m parquet.')
    tr_days, va_days, te_days = F.train_val_test_by_day(merged)
    all_cols = [c for c in merged.columns if any(c.endswith(x) for x in ['close','volume','numberOfTrades','prev_return','prev_volatility','return','volatility'])]
    ...
    with open(P.processed_dir + '/meta.json','w') as f:
        json.dump({'feature_cols':feat_cols,'stock_list':stock_list,'count_cols':count_cols}, f)
