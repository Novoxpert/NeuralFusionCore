...
class MarketNewsFusionWeightModel(nn.Module):
    def __init__(self, ts_input_dim, num_stocks, d_model=64, nhead=4, num_layers=2,
        ...
        self.head = nn.Sequential(nn.Linear(fused_dim, 64), nn.ReLU(), nn.Linear(64, num_stocks))

    def forward(self, ts_input, count_input, news_input):
        x = self.inp(ts_input); x = self.pos(x); x = self.ts_enc(x)[:, -1, :]
        n = self.news_proj(news_input); _, (hn, _) = self.news_lstm(n); n_emb = hn[-1]
        if self.count_lstm is not None:
            _, (hc, _) = self.count_lstm(count_input); c_emb = hc[-1]
