...
def rolling_backtest(pred_mu, pred_std, pred_corr, realized_returns,
                     dates, alpha=0.6, gamma=3.0, theta=0.90,
    ...
    for t in range(lookback_minutes, T, step):
        ...
        mu  = (1-alpha)*past_mu  + alpha*pred_mu[t]
        std = (1-alpha)*past_vol + alpha*pred_std[t]
        corr = (1-alpha)*past_corr + alpha*pred_corr[t]

        stats = {"mean":mu, "std":std, "corr":corr}
        w_m0 = method0_mvo(stats, gamma=gamma)
        w_m1 = method1_small_l2(stats, gamma=gamma, theta=theta)
        w_m2 = method2_subset_bagging(stats, gamma=gamma, num_resamples=num_resamples, subset_size=subset_size)
        list_weight0.extend(w_m0 for k in range(step))
        list_weight1.extend(w_m1 for k in range(step))
        list_weight2.extend(w_m2 for k in range(step))
        
        r_next = rets.iloc[t:t+step].to_numpy()
        eq0 += float((w_m0 * r_next).sum())
        eq1 += float((w_m1 * r_next).sum())
        eq2 += float((w_m2 * r_next).sum())

        ec0.append(eq0); ec1.append(eq1); ec2.append(eq2)
        w0.append(w_m0); w1.append(w_m1); w2.append(w_m2)
    ...
    weights = {
        "w0": np.vstack(w0), "w1": np.vstack(w1), "w2": np.vstack(w2),
        "dates": rets.index[lookback_minutes:lookback_minutes+len(w0)]
    }
