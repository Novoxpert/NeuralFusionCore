...
from lib.portfolio import rolling_backtest_pointwise
...
def main():
    ...
    mu, y = infer_full(model, te_loader, device=device)   # [T,S], [T,S]
    ...
    curves = rolling_backtest_pointwise(pred_mu=mu, realized_returns=y_main, dates=pd.DatetimeIndex(dates),
                                        alpha_mu=PC.alpha_mu, gamma=PC.gamma, theta=PC.theta,
                                        num_resamples=PC.num_resamples, subset_size=PC.subset_size,
                                        lookback_minutes=FC.fwd_h, step=FC.fwd_h)

    plot_portfolios(curves, out_png=Paths.outputs_dir + "/equity_curves_pointwise.png")
    print("Done. Curves plotted to data/outputs/equity_curves_pointwise.png")
