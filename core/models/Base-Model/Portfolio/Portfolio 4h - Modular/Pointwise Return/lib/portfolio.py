...
def rolling_backtest_pointwise(pred_mu, realized_returns, dates,
                               alpha_mu=0.6, gamma=3.0, theta=0.90,
                               num_resamples=20, subset_size=5, lookback_minutes=240, step=1):
    """
    Use model pointwise mu per asset; use HISTORICAL vol and corr to build covariance.
    Blend mu (only) with alpha_mu; sigma/corr are purely historical.
    """
    ...
    for t in range(lookback_minutes, T-1, step):
        ...
        mu_t   = pred_mu[t]
        mu_mix = (1 - alpha_mu)*past_mu + alpha_mu*mu_t
